...
**Date:** 2026-08-25
**Branch:** `feature/spec-v3-accuracy-conformance` (rebased onto main `d95bf0f` — includes the 2026-08-25 frontend design-system swap — and force-pushed through `d2e336c`)
...
**SDD workspace (gitignored):** `.superpowers/sdd/2026-08-25-spec-v3-accuracy-conformance/` — ledger (`progress.md`), task briefs `task-N-brief.md`, review packages. Regenerated 2026-08-25 on the saada machine from git + this doc (the original scratch lived on the Huzaifa clone); all 14 briefs + the T2 review package verified present.
**Execution mode:** Subagent-Driven Development, parallel across three disjoint streams
**Validation (2026-08-25, saada machine):** all 8 commit SHAs verified; every code anchor in the plan checked (scale.py resolver, router/vector/extraction wiring, phase2 assumed pin, AssumedScaleBanner + types + mount, spec §7.3 amendment + changelog, migration head `c37396f6713e`); backend suite **318 passed + 1 xfailed**, frontend **63/63** — plan and handoff claims hold.

---

## 1. Read this first (resume protocol)

1. Read the ledger: `.superpowers/sdd/2026-08-25-spec-v3-accuracy-conformance/progress.md`. Its first line names this plan; every `Task N: complete` line is done work — do not redo it.
2. **Immediate next action:** Task 2 is implemented and controller-ratified but its REVIEW has not run. The review package is at `.superpowers/sdd/2026-08-25-spec-v3-accuracy-conformance/task-2-review-package.diff` (commit `7a72def`, diff vs base `44c72b2`). Dispatch a task reviewer (brief = `task-2-brief.md`, report = `task-2-report.md`, package path above). On clean verdict: append `Task 2: complete` to ledger, push, then continue with Task 3.
...
### This machine (user saada — validated 2026-08-25, authoritative where they conflict)

- Git Bash on PATH — SDD helper scripts run directly (`bash <skill>/scripts/<script>`).
- Backend venv at `backend/.venv` (Python 3.13.1, full deps incl. opencv-python-headless). From `backend/`: `.venv/Scripts/python.exe -m pytest -q` (also works as `python -m pytest` with the venv activated).
- opencv IS installed → `tests/test_spike_raster_reproof.py` collects and passes; the "cv2 collection error" below does not apply here.
- Full suite: **318 passed + 1 xfailed** (2026-08-25). Frontend: **63/63** via `bun run test`.
- `.githooks` pre-commit active; repo-wide CRLF-vs-prettier friction — normalize only files you stage. Pre-existing eslint warning at EstimateClient.tsx.

### Huzaifa clone (where the original session ran — kept for reference)
...
- Pre-existing failures NOT caused by this work: `tests/test_spike_raster_reproof.py` collection error (`cv2` missing — opencv-python-headless not installed there) and one `alembic.__main__` migration test. Full suite otherwise reported 314 passed at T1-fix time.
- `.githooks` pre-commit IS active on that clone (core.hooksPath set 2026-08-25): eslint+tsc+prettier run on staged frontend commits; keep staged TS prettier-clean.
