...
- [ ] **Phase 2.5 — Spec v3 alignment** (see `Phases.md` for full DoD): Input Quality Gate + flattened-sample spike; clustering migration DBSCAN → distance-threshold union-find; raster spike (classical-CV template matching vs vector ground truth); quarantine ultralytics; `source_quality` Alembic migration; review-time instrumentation + `/review-metrics`. ~~declare or drop scikit-learn~~ ✅ declared 2026-08-22.
- [ ] Restore sample PDF to `data/samples/` from the project owner (gitignored by design; absent locally → 13 regression tests fail until restored).
...
- **Environment (fixed 2026-08-22):** `backend/.venv` recreated with all deps; `python-multipart` + `scikit-learn` now declared in `pyproject.toml`; `backend/.env` recreated from `.env.example`. Sample PDF still absent from `data/samples/` (gitignored client drawing; obtain from owner — 13 regression tests fail by design until then). Heavy ML deps (`ultralytics`, `detectron2`, `paddleocr`) are intentionally import-gated optionals — code degrades gracefully without them; install ad hoc only for the raster spike.
