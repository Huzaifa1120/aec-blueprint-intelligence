...
quantity, confidence_status, size_source and the unpriced flag; unpriced
lines show the review label in both cost cells — never $0.
"""
...
def render(rows: dict) -> bytes:
    ...
    if totals:
        ...
        for label, key in (
            ...
            if key in totals:
                ws.append([label, None, None, totals[key], None, None, None])
    buffer = io.BytesIO()
    wb.save(buffer)
