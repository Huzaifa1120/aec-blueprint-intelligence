...
def test_boq_lines_default_derived(db: OrmSession):
    line = _line(db)
    assert line["confidence_status"] == "DERIVED"
    assert line["confidence_score"] == 0.8


def test_size_assumed_downgrades_to_assumed(db: OrmSession):
    line = _line(db, size_source="assumed")
    assert line["confidence_status"] == "ASSUMED"
    assert line["confidence_score"] == 0.3


def test_scale_assumed_downgrades_to_assumed(db: OrmSession):
    line = _line(db, scale_assumed=True)
    assert line["confidence_status"] == "ASSUMED"


def test_degraded_multiplier_composes_with_derived(db: OrmSession):
